...
import { betaRegister } from "@/app/actions/beta";

const THEMES = {
  default: {
    bg: "#fdf8f0", dark: "#3b2a1a", mid: "#7c5c3e", pale: "#e8d5b7",
  ...
  warm: {
    bg: "#FDF0EB", dark: "#5C3030", mid: "#967878", pale: "#F0CCB8",
  ...
  cool: {
    bg: "#EEF2F8", dark: "#2D3250", mid: "#7B8DB0", pale: "#C8D4E8",
  ...
  humanities: {
    bg: "#F5F0E4", dark: "#3D2D14", mid: "#8A6A40", pale: "#E0CCA0",
...
const SUBJECT_THEME: Record<string, ThemeKey> = {
  "Русский язык": "warm", "Иностранный язык": "warm", "Литература": "warm",
  "Математика": "cool", "Физика": "cool", "Химия": "cool", "Биология": "cool", "Информатика": "cool",
...
export default function RegisterPage() {
  ...
  async function doRegister() {
    if (!betaCode.trim()) { setError("Введите код доступа к бете"); return; }
    setLoading(true);
    setError(null);

    // Всё делается на сервере: проверка кода, создание аккаунта, PRO план
    const result = await betaRegister(betaCode, email, password, name, subject);
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden"
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="mt-3 text-sm" style={{ color: theme.mid, transition: "color 0.7s ease" }}>
        ...
        <div className="rounded-2xl p-8 border" style={{
          ...
          borderColor:    theme.pale,
          boxShadow:      `0 2px 24px ${theme.shadow.replace("0.30", "0.10").replace("0.35", "0.10")}`,
          ...
          <form onSubmit={e => { e.preventDefault(); doRegister(); }} className="space-y-4">
            ...
            <div>
              ...
              <select value={subject} onChange={e => setSubject(e.target.value)} required
                className="w-full px-4 py-2.5 rounded-xl border outline-none"
                style={{ ...inputStyle, color: subject ? theme.dark : theme.mid }}>
          ...
          <p className="text-center mt-4 text-sm" style={{ color: theme.mid, transition: "color 0.7s ease" }}>
            Уже есть аккаунт?{" "}
            <Link href="/auth/login" className="font-semibold hover:underline" style={{ color: theme.dark }}>
              Войти
            </Link>
          </p>
        </div>
