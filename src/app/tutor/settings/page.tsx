...
import { saveMeetingUrl, saveBoardProfile } from "@/app/actions/settings";
import { Video, Check, LayoutTemplate } from "lucide-react";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      supabase.from("tutors")
        .select("meeting_url, subject_profile, board_bg")
        .eq("id", user.id)
        .single()
        .then(({ data }) => {
  ...
  async function handleSaveUrl(e: React.FormEvent) {
    e.preventDefault();
    setSavingUrl(true); setErrorUrl(null); setSavedUrl(false);
    const res = await saveMeetingUrl(meetingUrl);
    setSavingUrl(false);
    if (res.error) { setErrorUrl(res.error); return; }
    setSavedUrl(true); setTimeout(() => setSavedUrl(false), 3000);
  }
    ...
    <div className="max-w-lg space-y-6">
      ...
      <div className="rounded-2xl border p-6" style={card}>
        ...
        <p className="text-sm mb-4" style={{ color: "var(--brown-mid)" }}>
          Введите постоянную ссылку — Zoom, Google Meet, Яндекс Телемост или любую другую.
        ...
        {loading ? (
          ...
          <form onSubmit={handleSaveUrl} className="space-y-3">
            ...
            {errorUrl && <p className="text-sm text-red-600">{errorUrl}</p>}
            <button type="submit" disabled={savingUrl}
              className="flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold text-white"
              style={{ background: "var(--gradient-primary)", boxShadow: "var(--shadow-button)", opacity: savingUrl ? 0.7 : 1 }}>
              {savedUrl ? <><Check size={15}/> Сохранено</> : savingUrl ? "Сохраняем..." : "Сохранить"}
            </button>
          </form>
        )}
      </div>
